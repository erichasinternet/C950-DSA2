...
with open("data/packages.csv", mode='r', encoding='utf-8-sig') as packages_csv:
    CSV_Package = csv.reader(packages_csv)
...
def load_package_data(filename, package_hash_table):
    with open(filename, mode='r+', encoding='utf-8-sig') as package_info:
...
truck1 = trucks.Truck(16, 18, None, [1, 13, 14, 15, 16, 20, 29, 30, 31, 34, 37, 40], 0.0, wgu_address,
                     datetime.timedelta(hours=8))

# Create truck object truck2
truck2 = trucks.Truck(16, 18, None, [3, 6, 12, 17, 18, 19, 21, 22, 23, 24, 26, 27, 35, 36, 38, 39], 0.0,
                     wgu_address, datetime.timedelta(hours=10, minutes=20))

# Create truck object truck3
truck3 = trucks.Truck(16, 18, None, [2, 4, 5, 6, 7, 8, 9, 10, 11, 25, 28, 32, 33], 0.0, wgu_address,
...
def delivering_packages(trucks):
    ...
    while len(undelivered) > 0:
        ...
        for package in undelivered:
            if distance_in_between(extract_address(trucks.address), extract_address(package.address)) <= next_address:
                next_address = distance_in_between(extract_address(trucks.address), extract_address(package.address))
